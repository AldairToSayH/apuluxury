...
import { getMyOrderDetail } from "@/lib/ordersApi";
...
function BuyerOrderDetailContent() {
  ...
  useEffect(() => {
    ...
    getMyOrderDetail(token, params.id)
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-12">
      <div className="mb-8">
        <Link className="font-bold text-mineral hover:text-ink" href={routes.buyerOrders}>
          Volver a pedidos
        </Link>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-md bg-white p-6 shadow-soft">
            ...
            <div className="mt-6 grid gap-4 border-t border-ink/10 pt-5 text-sm text-ink/70 md:grid-cols-2">
              ...
              <div>
                <p className="font-bold text-ink">Vendedores</p>
...
export default function BuyerOrderDetailPage() {
  return <ProtectedPage role="buyer">{() => <BuyerOrderDetailContent />}</ProtectedPage>;
}
